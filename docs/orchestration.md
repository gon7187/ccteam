...
- **Commander & crews** (总控-工班) — Claude Fable at `high` effort decomposes, delegates, watches and accepts; it sizes every top-level task first: small (≤3 files, no hard trigger) runs one lane → GLM pre-gate → one Codex Sol gate with Claude out of the loop; medium gets a one-page Fable plan, one Sol review round, lane implementers, a GLM pre-gate and a single fresh Sol gate; large runs the full pipeline — a read-only OpenCode GLM (`zai-coding-plan/glm-5.3-flash`) scout brings 2–5 pinned GitHub precedents (URL, commit/tag, path, license; exactly one Codex Luna scout fallback when no usable GLM session exists), Fable writes the plan to `.ccteam/plans/` (id / implementer / dependencies / files / definition of done / lane / hard-or-flexible / starting effort per task, Amendments log for later changes), a fresh Codex Sol gates the plan (two rounds max, then a three-paragraph report to the human) and stays as the implementers' advisor, three vendor lanes (Codex Luna on backend/money/ETL, Claude Sonnet on frontend/routes/glue, GLM on tests/docs/boilerplate) execute in per-task git worktrees capped at 3 Claude / 3 Codex / 5 GLM, a GLM pre-gate (lint, targeted tests, secret scan, plan checklist) runs before a GLM git agent integrates and merges after a fresh Fable + Sol dual gate (single fresh Sol gate for small/medium) approves the same revision and local checks are green. Effort rides the ceiling `status` declares and climbs one rung per bounce, never below `high` for hard tasks; flexible tasks route to the vendor with the smallest `tokens_24h` share from `status` (15 % skew threshold, quota windows above 80 % override); four typed triggers (capability error, two `server_overloaded` in a row, 30 minutes of silence, a report missing `Status: done`) move a role to its fallback vendor once per task. The commander checks host load and session liveness at every decision point. The expensive model pays only for sizing, decomposition, gating and acceptance — volume work rides cheaper specialists.
