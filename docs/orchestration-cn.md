...
- **总控-工班** —— 强推理总控做规划/拆解/验收;每个顶层编码任务首次改码前,由只读 OpenCode GLM(`zai-coding-plan/glm-5.3-flash`)侦察员回报 2–5 个钉版 GitHub 先例(URL、commit/tag、路径、license),仅当没有可用 GLM 会话时才有恰好一次 Codex Luna 侦察兜底;codex 开发,完成通知回流总控,Opus+Sol 终审门不变。贵模型只花在拆解与验收上,量活走便宜的专长工。
