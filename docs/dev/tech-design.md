...
| **curated MCP + evolution**(v0.8.24 C)| curated MCP = `execution/mcp_config.rs` + stream-json spawn 写 `chat/<sid>/mcp.json`· evolution = `routes/evolution.rs`(读 `experience.jsonl`;聚合含 steered / per-vendor 分层 / invoked 子集)· invoked-skill 检测 = pump 侧确定性匹配(schema 家 = `progress_bridge::ChatTurnCompletionMetadata.invoked_skills` + `experience::TurnExperience.invoked_skills`)| `cargo test -p ccteam-web --test evolution_test` + openapi;harness/im lib tests |
