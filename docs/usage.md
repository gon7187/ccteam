...
Telegram also has a persistent quick-template keyboard. Send `/keys` to show the configured templates, tap one to arm its prefix for the next plain message, or send `/keys off` to remove the keyboard and discard an armed prefix. The default templates are Commander, Driver+advisor, Cross review, Bake-off, Triangulate, and Pyramid. The Commander template opens on Claude Fable at `high` effort, who first calls `status` and sizes the top-level task: small (≤3 files, no hard trigger — money, secrets, credentials, scope/ACL, data migration, project red line) runs a single lane, a GLM pre-gate, and one Codex Sol gate with Claude out of the loop entirely; medium gets a one-page Fable plan, one Sol review round, lane implementers, and a single fresh Sol gate; large runs the full pipeline — a read-only OpenCode GLM (`zai-coding-plan/glm-5.3-flash`) scout returns 2–5 pinned GitHub precedents (URL, commit/tag, path, license; exactly one Codex Luna scout fallback when no usable GLM session exists); Fable writes the plan to `<project>/.ccteam/plans/<date>-<topic>.md` (every task carries id, implementer, dependencies, files, definition of done, lane, hard-or-flexible, starting effort; changes after approval go to an Amendments section); a fresh Codex Sol gates the plan with a cap of two rounds (one for medium) and then stays on as the implementers' advisor (mandatory before any plan deviation, advice not binding, disagreements escalate to the commander); lane A (Codex Luna: backend, money, ETL, complex logic), lane B (Claude Sonnet: frontend, routes, glue), and lane C (GLM: tests, docs, templated migrations, boilerplate) each execute in their own git worktree and `task/<id>` branch, capped at 3 Claude / 3 Codex / 5 GLM sessions in parallel; every branch passes a GLM pre-gate (lint, targeted tests, secret scan, plan checklist) before a GLM git agent merges the branches into `integration/<topic>`, runs the project's full checks, resolves conflicts, opens the PR and merges into main only when the gate(s) approve the same revision and local checks are green (a fresh Fable + fresh Sol dual gate for large tasks, the single fresh Sol gate for small/medium; CI only if the project actually runs one). Effort rides the ceiling `status` declares per vendor and climbs one rung after a gate/pre-gate bounce (never above the declared ceiling, boilerplate lanes stay low, hard tasks never below `high`); flexible tasks (most medium ones) route to whichever vendor's `tokens_24h` share from `status` is smallest, with a 15 % relative skew threshold and subscription-quota windows above 80 % used overriding the token share. Four typed triggers move a role to its fallback vendor exactly once per task: a capability error (`vendor_unavailable` / `model_unavailable` / `effort_unavailable`), two consecutive `server_overloaded` dispatch results, 30 minutes of silence with no pending approval, or a report missing the leading `Статус: готово` line. The commander checks host load and session liveness before every spawn and on every completion notification and stops delegates it no longer needs; the executor brief template lives at `<project>/.ccteam/briefs/executor.md`. Customize the templates in `~/.ccteam/config.yaml`; changes are picked up without restarting the daemon:
...
A session deciding whom to spawn never has to guess. One `status` call (the MCP tool, also exposed through the byte-identical discovery alias `grok_claude_codex_kimi`) answers with a **vendor panel** for the host your current project is bound to: which vendors are installed and their versions, an honest auth signal (`ready` / `not_ready` / `unknown` — sitting on PATH never masquerades as logged in), budget state, and whether the snapshot is fresh or stale. Each vendor line also carries its trailing-24h spend, tokens, and subscription quota windows, so a commander can balance flexible work by usage instead of guessing:

```text
claude   2.1.4  auth=ready  budget=ok  spend_24h=$1.23  tokens_24h=123456  quota=five_hour:42%,reset=2026-09-04T18:00Z;weekly:10%
```
