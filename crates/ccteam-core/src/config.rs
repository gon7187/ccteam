...
mod tests {
    ...
    fn default_im_quick_templates_include_six_templates() {
        ...
        assert!(!templates[0].prefix.contains("до 10 Luna"));
    ...
    fn commander_template_runs_plan_gate_worktrees_git_agent_and_monitoring() {
        ...
        for phase in order {
            let at = prefix
                .find(phase)
                .unwrap_or_else(|| panic!("Commander v2 lacks phase {phase}"));
            assert!(at >= last, "phase {phase} is out of order");
            last = at;
        }
        let step6 = prefix
            .find("Шаг 6, финальный гейт")
            .expect("step 6 present");
        ...
        for phrase in [
            ...
            assert!(gate.contains(phrase), "final-gate section lacks {phrase}");
        }
        for phrase in [
            // ростер
            ...
            "free -m",
            "session_list (activity, last_active, waiting_approval",
            "15 процентов",
            ...
            "Fable после одобрения плана",
            "исполнители после одобрения кода финальным гейтом",
            // fallback ролей
