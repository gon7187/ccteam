...
export const I18N: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    tplCommanderP:
      ...
      "如果无法启动可用的 GLM 会话（spawn 明确失败、模型不可用、已确认首个回复为空，或已确认回复不符合上述报告要求），恰好一次通过 Codex Luna 以最高已声明 effort 侦察；Luna 也失败时，停止编码并如实上报。spawn 或 dispatch 结果未知时，先用 session_list 与 session_collect 核对 GLM 会话是否存活且有输出；仅确认没有可用会话才使用 Luna。\n" +
