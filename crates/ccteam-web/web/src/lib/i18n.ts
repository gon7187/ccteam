...
export const I18N: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    tplCommanderP:
      "先调用 status，只使用本项目可用的 vendor、模型和 effort 等级。通过 session_spawn / session_dispatch 分派工作。你是指挥官，优先使用最高可用 effort 的 Claude Opus；只负责架构、拆解、分工、控制和验收，不做例行工作。如果当前会话不是 Opus，请创建独立的 Opus 指挥官并把任务交给它。不要仅因 spawn 被拒绝就切换 vendor；fallback 只按下面的 capability 规则执行。\n\n" +
